...
class Tree_Table2 extends Model
{
    ...
    {
        return $sponsor_id = DB::table('tree_table2')->where('user_id', $user_id)->value('sponsor');
    }
    ...
    {
        $users      = DB::table('tree_table2')->where('sponsor', $sponsor_id)->get();
        $index      = 0;
        $user_array = array();
    ...
    public static function getAllUpline($user_id)
    {
        ...
        $result = self::join('profile_infos', 'profile_infos.user_id', '=', 'tree_table2.placement_id')
            ->where('tree_table2.user_id', $user_id)
            ->select('tree_table2.leg', 'tree_table2.placement_id', 'tree_table2.type', 'profile_infos.package')
            ->get();
